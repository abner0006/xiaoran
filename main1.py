...
all_lines =  ["🕒更新时间,#genre#"] +[version]  +[about] +[daily_mtv]+read_txt_to_array('专区/about.txt') + ['\n'] +\
             ...
             ["🦜春晚直播,#genre#"] + sort_data(cw_dictionary,set(cw_lines))  + read_txt_to_array('专区/2025春晚.txt') + ['\n'] + \
...
## custom定制 
#custom_lines_xiaoran =  ["更新时间,#genre#"] +[version] + ['\n'] +\
#             ["央视频道,#genre#"] + sort_data(ys_dictionary,correct_name_data(corrections_name,ys_lines)) + ['\n'] + \
#             ["卫视频道,#genre#"]+ sort_data(ws_dictionary,correct_name_data(corrections_name,ws_lines)) + ['\n'] + \
#             ["数字频道,#genre#"] + sort_data(sz_dictionary,set(correct_name_data(corrections_name,sz_lines))) + ['\n'] + \
...
# NEW 将合并后的文本写入文件
new_output_file = "abner1.txt"
new_output_file_simple = "abner2.txt"

# custom定制 将合并后的文本写入文件
output_file_custom_xiaoran = "xiaoran.txt"
...
#    # 定制
#    with open(output_file_custom_xiaoran, 'w', encoding='utf-8') as f:
#        for line in custom_lines_xiaoran:
#            f.write(line + '\n')
#    print(f"合并后的文本已保存到文件: {output_file_custom_xiaoran}")
...
make_m3u(output_file_custom_xiaoran, output_file_custom_xiaoran.replace(".txt", ".m3u"))
...
custom_lines_xiaoran_hj = len(custom_lines_xiaoran)  
print(f"黑名单行数: {combined_blacklist_hj} ")
print(f"txt行数: {all_lines_hj} ")
print(f"other行数: {other_lines_hj} ")
print(f"custom_lines_xiaoran行数: {custom_lines_xiaoran_hj} ")
